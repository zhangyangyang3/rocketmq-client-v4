...
use crate::protocols::{ConvertUtil, mq_command};
...
impl QueryConsumerOffsetResponseHeader {
    pub async fn read_from_broker(broker_stream: &mut TcpStream, opaque: i32) -> i64 {
        ...
        if frame.e_len == 0 {
            warn!("read from QueryConsumerOffsetResponseHeader failed, code:{}, r_body:{:?}", frame.req_code, String::from_utf8(frame.r_body));
            return 0;
        }

        return match frame.header_serialize_method {
            mq_command::HEADER_SERIALIZE_METHOD_JSON => {
                info!("QueryConsumerOffsetResponseHeader e_body:{:?}",String::from_utf8(frame.e_body));
                0
            }
            _ => {
                let mut bytes = Bytes::from(frame.e_body);
                ...
                return ConvertUtil::convert_string_bytes_to_i64(body)
            }
        }
