use crate::protocols::mq_command::{MqCommand, HEADER_SERIALIZE_METHOD_PRIVATE};
use bytes::{Buf, Bytes};
use log::debug;
...
impl NotifyConsumerIdsChangedRequestHeader {
    pub fn convert_from_cmd(cmd: &MqCommand) -> Self {
        ...
        debug!(
            "NotifyConsumerIdsChangedRequestHeader, extend:{:?}",
        ...
        match cmd.header_serialize_method {
            ...
            _ => {
                let json =
                    serde_json::from_slice::<NotifyConsumerIdsChangedRequestHeader>(&body.to_vec())
                        .unwrap();
                return json;
            }
        }
