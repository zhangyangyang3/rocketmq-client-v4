...
use crate::protocols::header::send_message_request_header::{
...
impl Producer {
    ...
    pub async fn init(
        ...
        tokio::spawn(async move {
            loop {
                ...
                if send.is_err() {
                    warn!(
                        "send command to mq failed. req cmd:{}, response error:{:?}",
                        cmd.req_code,
                        send.err()
                    );
                    continue;
                ...
                if resp.req_code != 0 {
                    warn!(
                        "send command to mq failed. req cmd:{}, response code:{}, remark{:?}",
                        cmd.req_code,
                        resp.req_code,
                    ...
                    debug!(
                        "read from mq success. req cmd:{}, resp remark:{:?}, ext field:{:?}",
                        cmd.req_code,
                        String::from_utf8(resp.r_body),
        ...
        tokio::spawn(async move {
            loop {
                let heart_beat =
                    HeartbeatData::new_producer_data(client.clone(), producer_group.clone());
    ...
    pub async fn send_message_with_tag(
        &mut self,
        topic: String,
        tag: String,
        message: Vec<u8>,
    ...
    pub async fn send_with_properties(
        ...
        let queue_id: i32 = (get_current_time_millis()
            % self.message_queue_map.get(&topic).unwrap().len() as i64)
            as i32;
        ...
        let cmd =
            MqCommand::new_with_body(request_code::SEND_BATCH_MESSAGE, vec![], header_bytes, body);
...
mod send_test {
    ...
    async fn send_message_test() {
        ...
        for _i in 0..10 {
            let uid = Uuid::new_v4();
            producer
                .send_message(topic.clone(), body.clone(), uid.to_string())
                .await
                .unwrap();
    ...
    fn to_string_test() {
        let arr: [u8; 166] = [
            0, 1, 97, 0, 0, 0, 17, 114, 117, 115, 116, 95, 115, 101, 110, 100, 95, 103, 114, 111,
            117, 112, 95, 49, 0, 1, 98, 0, 0, 0, 14, 116, 111, 112, 105, 99, 95, 116, 101, 115,
            116, 95, 48, 48, 55, 0, 1, 99, 0, 0, 0, 6, 84, 66, 87, 49, 48, 50, 0, 1, 100, 0, 0, 0,
            4, 49, 48, 48, 48, 0, 1, 101, 0, 0, 0, 1, 54, 0, 1, 102, 0, 0, 0, 1, 48, 0, 1, 103, 0,
            0, 0, 13, 49, 55, 50, 55, 49, 52, 51, 52, 52, 52, 57, 57, 48, 0, 1, 104, 0, 0, 0, 1,
            48, 0, 1, 105, 0, 0, 0, 6, 75, 69, 89, 83, 49, 48, 0, 1, 106, 0, 0, 0, 1, 48, 0, 1,
            107, 0, 0, 0, 5, 102, 97, 108, 115, 101, 0, 1, 108, 0, 0, 0, 1, 48, 0, 1, 109, 0, 0, 0,
